// const object = fetch("https://google-news.p.rapidapi.com/v1/top_headlines?lang=en&country=US", {
// 	"method": "GET",
// 	"headers": {
...
function renderNews(article = []) {
    if (typeof object === 'object') {
        for (let i = 0; i < 10; i++) {
            const feeder = generateNewsFeed(article[i]);
            newsFeed.innerHTML += feeder;
        }
        deleteNewsError();
        renderArticles(article.sub_articles);
    }
}

function renderArticles(sub = []) {
...
function deleteNewsError() {
    const mistakes = document.getElementById("news-error");
    mistakes.remove();
}
